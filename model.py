...
class BE_sub(nn.Module):
    def __init__(self, inputsize):
        ...
        self.BE_gru = BGRU(inputsize, 128, 2)
        self.dropout = nn.Dropout(p=0.1)
        # self.BE_attention = SelfAttention(128 * 2)
        self.BE_fcnn = FCNN(31 * 256, 0.1)
...
class Blosum62_sub(nn.Module):
    def __init__(self, inputsize):
        ...
        self.dropout = nn.Dropout(p=0.1)
        self.Blosum62_lstm = BLSTM(inputsize, 64, 2)
        self.Blosum62_att = SelfAttention(64 * 2)
        self.Blosum62_fcnn = FCNN(31 * 128, 0.1)
...
class AAI_sub(nn.Module):
    def __init__(self, inputsize):
        ...
        self.dropout = nn.Dropout(p=0.1)
        self.AAI_gru = BGRU(inputsize, 64, 3)
        self.AAI_att = SelfAttention(64 * 2)
        self.AAI_fcnn = FCNN(31 * 128, 0.1)
...
class CKSAAP_sub(nn.Module):
    def __init__(self, inputsize):
        ...
        self.dropout = nn.Dropout(p=0.1)
        self.CKSAAP_cnn1 = CNN(inputsize, 64, 3, 2)
        ...
        self.h_r1 = nn.Conv2d(64, 64, kernel_size=1, stride=1, bias=False)
        self.h_r2 = nn.Conv2d(64, 128, kernel_size=1, stride=1, bias=False)
        self.GAP = nn.AvgPool2d(kernel_size=(2, 2))
        # self.CKSAAP_att = SelfAttention(256)
        self.CKSAAP_fcnn = FCNN(256, 0.1)
